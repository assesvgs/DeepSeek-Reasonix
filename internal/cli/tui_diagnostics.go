...
const (
	...
	// (the ticker fires every tuiWatchdogInterval). A larger gap means the
	// whole process was suspended by the OS (Android backgrounding, screen
	// off): the heartbeat age measured at resume is inflated by the pause and
	// must not be trusted to kill the TUI — see watchdogTickGap.
	...
	for {
		...
		case now := <-ticker.C():
			if watchdogTickGap(lastTick, now) {
				...
				// so the heartbeat age is inflated by the pause, not by a
...
// watchdogTickGap reports whether the gap since the previous watchdog tick
// proves the whole process was suspended by the OS. A ticker gap far beyond
// its 1s interval (suspendAllowance) means the process was frozen, so the
// heartbeat age read at resume is inflated by the pause and must not escalate.
func watchdogTickGap(prevTick, now time.Time) bool {
	return !prevTick.IsZero() && now.Sub(prevTick) > suspendAllowance
}

// noteSuspended refreshes the heartbeat after an OS suspension. The pause
// inflated the wall/monotonic age of the last heartbeat; resetting it to the
// resume moment keeps the watchdog from misreading a healthy-but-asleep TUI
// as a wedged event loop. A real stall after resume still escalates normally
// because this only resets the reference point, not the escalation rules.
func (d *tuiDiagnostics) noteSuspended(now time.Time, gap time.Duration) {
	if d == nil {
		return
	}
	d.mu.Lock()
	...
	d.lastHeartbeat = d.now()
	d.lastHeartbeatSource = "resumed_from_suspend"
	d.logfLocked("watchdog_state phase=%s gen=%d source=suspended tick_gap=%s liveness_refreshed",
		d.phase, d.generation, gap)
}
